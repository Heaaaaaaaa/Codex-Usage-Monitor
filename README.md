...
The development Git history contains early machine-local metadata and must not be pushed directly to a public repository. `make verify-public-source` audits the current tracked snapshot, while `make source-archive` creates a clean source ZIP for seeding the public repository after a public commit identity and license have been selected. See [PUBLISHING.md](PUBLISHING.md) for the sanitized-history workflow.
